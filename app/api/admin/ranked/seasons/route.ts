...
export async function GET(request: NextRequest) {
  ...
  const seasons = await listRankedSeasons();
...
export async function POST(request: NextRequest) {
  ...
  try {
    const season = await createRankedSeason({
      ...
      resetAt: parsed.data.resetAt ? new Date(parsed.data.resetAt) : null,
    ...
    await logAdminAction({
      adminId: admin!.id,
      action: "RANKED_SEASON_CREATE",
      targetType: "ranked_season",
      targetId: season.id,
      summary: `Criou temporada ${season.name} (#${season.seasonNumber})`,
    });
