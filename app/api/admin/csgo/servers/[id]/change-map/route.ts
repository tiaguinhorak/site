...
import { changeCsgoServerMap } from "@/lib/csgo-api/server-control";
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  const result = await changeCsgoServerMap(id, parsed.data.map);
  ...
  await logAdminAction({
    adminId: admin!.id,
    action: "CSGO_SERVER_CHANGE_MAP",
    targetType: "csgo_server",
    targetId: id,
    summary: `Alterou mapa CS:GO para ${parsed.data.map}`,
  });
