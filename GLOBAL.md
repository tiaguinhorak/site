...
- **Dynamic content (store, game modes, warmup, marketing, plans):** Same pattern via `lib/i18n/localize-dynamic.ts` — admin saves in PT; `buildGenericTranslations()` on write; lazy translate + persist on read for `en`/`es`. Models with `translations` JSON: `StoreItem`, `GameMode`, `GameModeRoom`, `WarmupMode`, `MarketingFeature`, `SubscriptionPlan`, `SiteStat`.
